create function fl.emit(p_topic text, p_entity uuid, p_project uuid, p_payload jsonb)
returns void language plpgsql as $$
begin
  perform pg_advisory_xact_lock(hashtext('events_outbox'));
...
create table events_retention (
  ...
  low_water_id bigint not null default 0
);

insert into events_retention (only_row, low_water_id) values (true, 0);
